...
export const telegramClient = {
  ...
  // Get QR login token for scanning with Telegram mobile app
  async getQRLoginToken(userId: string) {
    console.log(`Getting QR login token for user ${userId}`);
    
    // Get auth headers
    const headers = await this.getAuthHeaders();
    
    const { data, error } = await supabase.functions.invoke('telegram-auth/qr-login-token', {
      body: { userId },
      headers
    });
    
    if (error) {
      console.error("Error calling telegram-auth/qr-login-token:", error);
  ...
  async checkQRLoginStatus(userId: string, token: string) {
    console.log(`Checking QR login status for user ${userId}`);
    
    // Get auth headers
    const headers = await this.getAuthHeaders();
    
    const { data, error } = await supabase.functions.invoke('telegram-auth/check-qr-login', {
      body: { userId, token },
      headers
    });
    
    if (error) {
      console.error("Error calling telegram-auth/check-qr-login:", error);
